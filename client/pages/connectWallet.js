...
const connectWallet = () => {
  ...
  const {currentAccount , connectWallet } = useContext(NFTMarkitplaceContext);
    ...
    <div className={Style.connectWallet}>
      <div className={Style.connectWallet_box}>
        ...
        <p className={Style.connectWallet_box_para}>
          Connect with one of our avaliable wallet providers
          or create a new one
        </p>
        <div className={Style.connectWallet_box_provider}>
          {
            providerArray.map((el, i) => (
              <div className={`${Style.connectWallet_box_provider_item} ${activeBtn == i + 1 ? Style.active : ""}`}
                key={i + 1} onClick={() => (setActiveBtn(i + 1) , connectWallet())}>
                <Image
                  src={el.provider}
                  alt={el.provider}
                  width={50} height={50}
